...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://pro.kagem.shop", "http://localhost:3000"],
...
slots = load_slots("parking_slots.csv")
...
def stream_video(speed: int = 1): # speed 쿼리 파라미터 추가 (기본 1배속)
    ...
    def generate():
        ...
        while cap.isOpened():
            ...
            # YOLO 감지
                results = model.predict(frame, imgsz=1280, conf=0.3, iou=0.4, classes=[0], verbose=False)
