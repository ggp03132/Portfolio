...
// API Base URL 설정 (프로덕션에서는 /api 사용)
...
function App() {
  ...
  const [totalSlots, setTotalSlots] = useState(202); 
  const [remainingSlots, setRemainingSlots] = useState(202);
  ...
  // [변경 2] 폴링 간격 조정 (실시간성을 위해 0.5초로 단축 추천)
  const startPolling = () => {
    if (pollingRef.current) clearInterval(pollingRef.current);

    pollingRef.current = setInterval(async () => {
      try {
        // 백엔드에서 전역 변수에 저장된 최신 값을 가져옴
        const res = await fetch(`${API_BASE_URL}/parking_spaces`);
    ...
    }, 500); // 1000ms -> 500ms로 변경하여 반응 속도 높임
  };
          ...
          <>
            <section className="top-section">
              ...
              <div className="card preview-card">
                {showStream ? (
                  // [변경 4] 스트림 URL에 speed 파라미터 추가 및 key 설정
                  // key에 speed를 넣으면 속도가 바뀔 때 컴포넌트가 다시 마운트되어 즉시 반영됨
                  <img
                    key={playbackSpeed} 
                    src={`${API_BASE_URL}/stream?speed=${playbackSpeed}`}
                    alt="stream"
                    style={{
                      width: "auto",
                      height: "auto%",
